...
for cat in categories:
    ...
    while True:
        ...
        if int(tree.xpath('//totalresults')[0].text) == 0:
            break
        titles = [e.text for e in tree.xpath('//feed/entry/title')]
        loc = [e.text for e in tree.xpath('//feed/entry/where/entrylink/entry/title')]
        pos = [e.text for e in tree.xpath('//feed/entry/where/point/pos')]
